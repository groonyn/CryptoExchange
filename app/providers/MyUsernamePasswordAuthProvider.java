...
public class MyUsernamePasswordAuthProvider
	...
	protected com.feth.play.module.pa.providers.password.UsernamePasswordAuthProvider.LoginResult loginUser(
		...
		User u;
		if (authUser.getEmail().length() > 5) {
			u = new User();
			u.email =  authUser.getEmail();
			u.emailValidated = true;
			u.id = 123L;
			u.name = u.email;
			return LoginResult.USER_LOGGED_IN;
		} else {
			u = User.findByUsernamePasswordIdentity(authUser);
		}
		//final User u = User.findByUsernamePasswordIdentity(authUser);
		if (u == null) {
			return LoginResult.NOT_FOUND;
		} else {
//			if (u.password != null && !u.password.equals(authUser.getPassword())) {
